...
const MainScreen = () => {
  ...
  getUser = async () => {
    try {
      const response = await user.getUserAccount();
      if (response.data) {
        setFetchedUser(response.data);
        setValue(response.data.specialty.id);
  ...
  return (
    <KeyboardAvoiding>
      <SafeAreaView className="flex-1 pt-5 bg-white">
        ...
        <View className="mt-1">
          <View styles={{ marginVertical: 5, flexDirection: "row" }}>
            {fetchedUser && <UploadImage
              img={
                fetchedUser
                  ? fetchedUser.imgUrl
                  : `https://ui-avatars.com/api/?background=f87171&color=fff&name=${fetchedUser ? fetchedUser.full_name : 'Null'}`
              }
              id={fetchedUser.id}
            />}
